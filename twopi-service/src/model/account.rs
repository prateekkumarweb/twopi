...
impl AccountModel {
    ...
    pub async fn insert(self, db: &DbConn) -> Result<Uuid, DbErr> {
        let result = Account::insert(ActiveModel {
            ...
            #[allow(clippy::unwrap_used)]
            account_type: ActiveValue::Set(serde_json::to_string(&self.account_type).unwrap()),
            currency_code: ActiveValue::Set(self.currency_code),
            starting_balance: ActiveValue::Set(self.starting_balance),
            created_at: ActiveValue::Set(self.created_at),
            account_extra: ActiveValue::Set(self.account_extra),
        })
        .exec(db)
        .await?;
    ...
    pub async fn upsert_many(accounts: Vec<Self>, db: &DbConn) -> Result<(), DbErr> {
        Account::insert_many(accounts.into_iter().map(|a| ActiveModel {
            ...
            account_extra: ActiveValue::Set(a.account_extra),
        }))
        .on_conflict(
